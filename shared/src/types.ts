...
export interface GeoPoint {
  lat: number;
  lng: number;
  alt: number; // meters above ground
}
...
export interface DroneHealth {
  ...
  temperatureC: number;
  motorRpm: number;
  vibrationScore: number; // 0 - 100
  signalStrengthDbm: number; // e.g. -45 to -95 dBm
...
export interface MissionTask {
  ...
  scoreBreakdowns?: Record<string, TaskScoreBreakdown>; // droneId -> score
}

export type ObjectDetectionType =
  | 'PERSON'
  | 'VEHICLE'
  | 'DAMAGED_BUILDING'
  | 'FIRE_SMOKE'
...
export interface AnalyticsMetrics {
  ...
  timeSeriesCoverage: { timestamp: number; coverage: number; activeDrones: number }[];
  timeSeriesBatteryDraw: { timestamp: number; avgBattery: number }[];
