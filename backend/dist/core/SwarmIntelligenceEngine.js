...
exports.SwarmIntelligenceEngine = void 0;
const shared_1 = require("@swarmos/shared");
class SwarmIntelligenceEngine {
    weights = { ...shared_1.DEFAULT_ALGORITHM_WEIGHTS };
     ...
     */
    scoreDroneForTask(drone, task, allTasks) {
        // 1. Hard disqualification conditions
        if (drone.status === 'FAILED' ||
            drone.status === 'DISCONNECTED' ||
            drone.status === 'EMERGENCY' ||
            drone.battery <= shared_1.SAFETY_PARAMETERS.criticalBatteryThresholdPercent) {
            return {
                priorityScore: 0,
                distanceScore: 0,
                batteryScore: 0,
        ...
        if (drone.battery < shared_1.SAFETY_PARAMETERS.warningBatteryThresholdPercent) {
        ...
        const currentWorkloadCount = allTasks.filter((t) => t.assignedDroneId === drone.id && t.status !== 'COMPLETED' && t.status !== 'FAILED').length;
        const workloadPenalty = currentWorkloadCount * 15;
        // Weighted Total Score Calculation
        const totalScore = (this.weights.priorityWeight / 100) * priorityScore +
     ...
     */
    allocateTask(task, drones, allTasks, isReallocation = false, reasonContext) {
        ...
        if (!bestDroneId) {
            return {
                ...
                decisionLog: {
                    ...
                    why: `All connected drones are either occupied, disqualified by critical battery (<${shared_1.SAFETY_PARAMETERS.criticalBatteryThresholdPercent}%), or unreachable.`,
...
exports.SwarmIntelligenceEngine = SwarmIntelligenceEngine;
