...
exports.NetworkTopologyEngine = void 0;
const shared_1 = require("@swarmos/shared");
class NetworkTopologyEngine {
    activeRelayDroneId = 'drone-03';
    calculateDistanceMeters(p1, p2) {
        const R = 6371000;
    ...
    computeTopology(drones) {
        ...
        if (!relayIsHealthy && drones.length > 0) {
            ...
            const bestCandidate = drones
                .filter((d) => d.id !== this.activeRelayDroneId &&
                d.status !== 'FAILED' &&
            ...
            if (bestCandidate) {
                ...
                bestCandidate.altitude = shared_1.SAFETY_PARAMETERS.relayAltitudeMeters; // Climb to relay altitude
                if (bestCandidate.targetPosition) {
                    bestCandidate.targetPosition.alt = shared_1.SAFETY_PARAMETERS.relayAltitudeMeters;
                }
                decisionLogs.push({
                    id: `log-net-${Date.now()}`,
                    timestamp: Date.now(),
                    ...
                    decision: `Commanded ${bestCandidate.callsign} to climb to ${shared_1.SAFETY_PARAMETERS.relayAltitudeMeters}m and activate High-Bandwidth Mesh Repeater.`,
                });
            }
        }
        // Build Nodes
        const gcsNode = {
            ...
            type: 'COMMAND_CENTER',
            position: shared_1.COMMAND_CENTER_LOCATION,
            status: 'ONLINE',
            isRelay: false,
            battery: 100,
        };
        ...
        for (const drone of drones) {
            ...
            // Distance to GCS
            const distToGcs = this.calculateDistanceMeters(drone.position, shared_1.COMMAND_CENTER_LOCATION);
            ...
            const isDirectFeasible = directSignalDbm > shared_1.SAFETY_PARAMETERS.commSignalLossThresholdDbm;
        ...
        const avgLatency = links.length > 0 ? Math.round(links.reduce((sum, l) => sum + l.latencyMs, 0) / links.length) : 35;
        const meshHealth = Math.round(Math.max(0, 100 - isolatedNodeIds.length * 25 - (avgLatency > 40 ? 15 : 0)));
        const coverage = Math.round(((drones.length - isolatedNodeIds.length) / Math.max(1, drones.length)) * 100);
        return {
            topology: {
                nodes,
                links,
...
exports.NetworkTopologyEngine = NetworkTopologyEngine;
