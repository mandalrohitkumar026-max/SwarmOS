...
export class CVDetectionEngine {
  ...
  public getAllDetections(): CVDetection[] {
  ...
  public registerDetection(
    ...
    const generatedTasks: MissionTask[] = [];
    ...
    if (isPerson && detection.status === 'UNCONFIRMED') {
      const verificationTask: MissionTask = {
