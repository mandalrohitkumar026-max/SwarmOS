...
export interface FailureRecoveryResult {
  affectedDrone: DroneTelemetry;
  reassignedTasks: MissionTask[];
  updatedTasks: MissionTask[];
  updatedSectors: DisasterSector[];
...
export class FailureRecoveryEngine {
  private intelligenceEngine: SwarmIntelligenceEngine;
  private recoveryTimes: number[] = [];

  constructor(intelligenceEngine: SwarmIntelligenceEngine) {
    this.intelligenceEngine = intelligenceEngine;
  }

  public getAverageRecoveryTimeSeconds(): number {
    if (this.recoveryTimes.length === 0) return 2.4;
  ...
  public handleDroneFailure(
    ...
    const unfinishedTasks = tasks.filter(
      (t) => t.assignedDroneId === failedDroneId && t.status !== 'COMPLETED'
    );

    const availableDrones = drones.filter(
      (d) => d.id !== failedDroneId && d.status !== 'FAILED' && d.status !== 'DISCONNECTED' && d.battery > 20
    );

    const reassignedTasks: MissionTask[] = [];
    ...
    for (const task of unfinishedTasks) {
      ...
      const alloc = this.intelligenceEngine.allocateTask(
        task,
        availableDrones,
        tasks,
        true,
        `Unit ${targetDrone.name} (${targetDrone.callsign}) entered ${newStatus}: ${reason}. Reallocating high-priority payload.`
      );
    ...
    const failureLog: AIDecisionLog = {
